...
REPO="Ryu0118/RemoteConfigGen"
BIN_NAME="remote-config-gen"
...
detect_platform() {
  ...
  case "$os" in
    Darwin) os="darwin" ;;
    *) error "unsupported OS: $os (remote-config-gen requires macOS)" ;;
  esac
...
main() {
  ...
  if [ -n "$current" ] && [ "$current" = "$clean_version" ] && [ -z "$FORCE" ]; then
    ...
    info "  curl -fsSL https://raw.githubusercontent.com/Ryu0118/RemoteConfigGen/main/install.sh | FORCE=1 bash"
    exit 0
  fi
